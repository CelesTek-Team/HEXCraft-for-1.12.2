...
/**
 * A baked model of a special hexagonal prism which uses a single texture and is rendered on the translucent layer. Supports quad caching, matrix and item perspective transforms
 */
@SideOnly(Side.CLIENT)
public class BakedModelEnergizedMonolith implements IBakedModel
{
	/**
	...
	*/
	...
	/**
	 * The matrix transforms generated by {@link ModelEnergizedMonolith}
	 */
	protected final Optional<TRSRTransformation> transform;

	protected final TextureAtlasSprite sprite;

	public BakedModelEnergizedMonolith(IModelState state, VertexFormat format, Optional<TRSRTransformation> transform, TextureAtlasSprite sprite)
	{
		this.state = state;
		this.format = format;
	...
	public List<BakedQuad> getQuads(IBlockState state, EnumFacing side, long rand) // Culling?
	{
		...
		// Draw the quads
		// Render if on the translucent layer
		if(renderLayer == null || renderLayer == BlockRenderLayer.TRANSLUCENT) HexShapes.Monolith.create(quads, this.format, this.transform, null, 0, false, this.sprite);
		return quads;
	}
