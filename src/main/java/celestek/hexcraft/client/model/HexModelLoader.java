...
import celestek.hexcraft.client.model.special.ModelEnergizedMonolith;
...
public class HexModelLoader implements ICustomModelLoader // Split into separate loaders for different models?
{
	...
	ENERGIZED_MONOLITH = "models/block/special/energized_monolith",
	...
	public boolean accepts(ResourceLocation location)
	{
		...
		return location.getResourceDomain().equals(HexCraft.ID) && (path.equals(LAYERED_CUBE) || path.equals(CONNECTED_LAYERED_CUBE) || path.equals(MONOLITH) ||path.equals(ENERGIZED_MONOLITH) ||path.equals(DOOR) || path.equals(DOUBLE_DOOR));
	}
	...
	public IModel loadModel(ResourceLocation location) throws Exception
	{
		...
		else if(path.equals(MONOLITH)) return new ModelMonolith(TEXTURE_MISSING, TEXTURE_MISSING, Optional.empty());
		else if(path.equals(ENERGIZED_MONOLITH)) return new ModelEnergizedMonolith(TEXTURE_MISSING, Optional.empty());
